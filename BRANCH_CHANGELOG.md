...
| **定位** | 相对 `3.1.7` 的后续迭代 |
| **基于** | `3.1.7` |
| **要点** | 流程伤害统计：全队/当前双列；记录进全局存储（一行：方案名悬停看全称与队伍，伤害列带首字）；最多 10 条。刷新从草稿恢复准备/流程，草稿缺 slots 时回落到已加载方案。准备阶段显示单次结算伤害。计算页顶部 3 槽切换 + 方案库；去掉编队主C勾选，面板/词条/最优看编辑中槽位；额外 Buff 按全队或角色1/2/3钉死槽位。顶栏淡蓝（未定死）。局外按人取值：转模/招式结算不再吃编辑中面板或残缺 `convertSlotPanels`；改词条走增益目录缓存少扫。修复：异化系数缺字段导致招式详情期望伤害为 —；局外/词条不再把角色基础的 1 写进异化系数 |
| **建议** | 日常开发与新功能提交落此分支 |
