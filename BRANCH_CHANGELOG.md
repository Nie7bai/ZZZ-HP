...
### `fix/calculator-anomaly-provider-source`（进行中）

| 项 | 说明 |
|---|---|
| **基线** | `main` |
| **目标** | 对齐普通面板与最优词条路径的异常强度提供者基础伤害来源 |
| **要点** | 提供者与持有者相同时沿用招式来源；异角色命破提供者使用贯穿力，其他职业使用攻击力；普通面板与最优路径共用同一规则，并补充身份规则矩阵及三处生产接线回归测试 |
| **状态** | 已实现并完成 red-green 回归验证，待提交 PR |
