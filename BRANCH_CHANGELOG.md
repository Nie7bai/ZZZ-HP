...
**当前开发线**：`3.1.7.2`（GitHub 同名分支；本地与 `upstream/3.1.7.2` 对齐后再改）。  
**上一开发线**：`3.1.7.1` / `3.1.6.3`（保留对照）。  
**稳定主干**：`main`（合入 `3.1.7`）。
...
| **定位** | 相对 `3.1.7` 的后续迭代 |
| **基于** | `3.1.7` |
| **要点** | 流程伤害统计：全队/当前双列；记录进全局存储（一行：方案名悬停看全称与队伍，伤害列带首字）；最多 10 条。刷新从草稿恢复准备/流程，草稿缺 slots 时回落到已加载方案。准备阶段显示单次结算伤害。计算页顶部 3 槽切换 + 方案库；去掉编队主C勾选，面板/词条/最优看编辑中槽位；额外 Buff 按全队或角色1/2/3钉死槽位。顶栏淡蓝（未定死）。局外按人取值：转模/招式结算不再吃编辑中面板或残缺 `convertSlotPanels`；改词条走增益目录缓存少扫。额外精通按槽位取值，不算进蕾米异化系数转模。修复：异化系数缺字段导致招式详情期望伤害为 —；局外/词条不再把角色基础的 1 写进异化系数 |
| **建议** | 对照 GitHub `3.1.7.1`；新工作落 `3.1.7.2` |

### `3.1.7.2`（当前开发线）

| 项 | 说明 |
|---|---|
| **定位** | GitHub `3.1.7.2`：置顶编队编辑、转模来源绿框 |
| **基于** | `3.1.7`（`a43a125`） |
| **package** | `3.1.7.2` |
| **要点** | 与 GitHub `9bf994b` 对齐（含计算器 Buff JSON）。切到最优词条时卸掉隐藏的面板结算 DOM，避免进页卡顿；计算方式 Tab 不再顺带平滑滚动。准备招式显示单次伤害（不含失衡），不算招式库、流程仍整套重算。「清空当前配置」只清页面上的方案内容（队伍/面板/Buff/准备流程/敌方），不删方案库存档。最优词条 4/5/6 只在点进该功能时带一次当时编辑中角色的词条主属性，进去后不再跟着换人/词条页更新。侧栏「面板计算 / 词条计算」合成「伤害计算」，只与最优词条互斥；三人导入各自选面板导入或词条计算，卡片打标签，结算按人 |
| **建议** | 日常开发落此分支 |
