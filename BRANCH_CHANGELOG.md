...
| **基于** | `3.1.7`（`a43a125`） |
| **package** | `3.1.7.2` |
| **要点** | 与 GitHub `9bf994b` 对齐（含计算器 Buff JSON）。切到最优词条时卸掉隐藏的面板结算 DOM，避免进页卡顿；计算方式 Tab 不再顺带平滑滚动。准备招式显示单次伤害（不含失衡），不算招式库、流程仍整套重算。「清空当前配置」只清页面上的方案内容（队伍/面板/Buff/准备流程/敌方），不删方案库存档。最优词条 4/5/6 只在点进该功能时带一次当时编辑中角色的词条主属性，进去后不再跟着换人/词条页更新 |
| **建议** | 日常开发落此分支 |
