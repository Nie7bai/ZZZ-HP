...
| **最优分配改自适应 Beam** | 单路贪心换成多路线并行；三参数「初始候选门槛 / 路线保留比例 / 最大保留路线」+ 快速/均衡/精细预设；游戏 4 口袋全跑 Beam | 见 `feature/affix-alloc-rework`（未提交） |
...
**现行搜索**：同一预算下保留**多条完整分配路线**，最后比总伤，再对前几名各跑一次换档。

- 按**已用词条数**（不是「加了几次」）分桶推进；每个存活状态扩展候选池里所有当前可用条目。
- 按**规范化分配**去重；同一档数下先按累计提升比例筛，再按**自适应 B** 截顶。
- **自适应 B** 由剩余计算量反推，预算紧时自然退化成单路；预算不足以完成整层时保留上一层完整状态（不留半成品）。
- Beam 结束后对前 `min(3, B)` 个状态各跑一次 1-swap / 2-swap，避免只优化第一名。
- **穿透专路**：已启用则锁 `main:slot5:penRate` / `set:penRate:8`（绕过门槛与 B），在带穿透率的面板上重测、形成独立候选池，再跑同一套 Beam，和普通路比最终总伤。
- 提升率两条路都用「一个词条都没分」的总伤当分母。

**三个用户参数**（取代「最低收益比例 + 候选宽度」）：

| 参数 | 含义 |
|---|---|
| 初始候选门槛 | 按**满额收益**（这条词条能投的档一次全投）永久淘汰太弱的条目 |
| 路线保留比例 | 同一档数下只留接近最好的那批分法 |
| 最大保留路线 B | 同时最多试多少种分法 |

**门槛为什么必须量「满额收益」**：量「1 档边际」时，cap=1 的主属性（24% 穿透 / 30% 增伤）占据分母，把「单档值小但能连吃多档」的副词条整批筛掉，46 档预算只花得掉 10 档、总伤腰斩。口径换成满额收益后恢复正常。

**预设**（真实 96 命中流程实测落定，三者都打到同一最优）：
...
**计算量**：`maxWorkUnits` 是总刹车，默认 **200000**。一次评估计价 = `1 + 命中数`，缓存命中不计入。自适应 B 按剩余预算反推，所以这 20 万既刹车也决定每层开几条路线。可手动停止。

**结果文案**：计算量用尽（Beam 没走完）标「截断」；候选太多跳过了可选的 1/2-swap 另标「跳过精修」，两者不再混为一谈。

**验证**：`test-affix-optimizer.mjs` 44 passed（含门槛 / 比例 / B 三组扫描与「快速 ≥ 精细 95%」「均衡 ≥ 精细 99%」两道质量闸）；`test-game-affix-rules.mjs` 54 passed（4 口袋实跑、按总伤取最高、各袋合计、5 号付费袋锁不到 24%、4 号位 6 条全进付费分支且各带 cap 5 税）；`scripts/test-*.mjs` 全过；`vue-tsc` / ESLint 过。fixture `zzz-hp/fixtures/pen-rate-alloc/ye-shiyuan-pen-rate.json`（「不带东西」）选出 24+8。
