...
## 6. 最优分配穿透专路（2026-09-16）

> 详细口径在开发手册 `dev-docs/词条最优分配.md`（不进仓库）。本节是仓库内摘要。

**要修的漏解**：5 号穿透 24% 和 2 件套穿透 8% 单独 +1 都不如同槽增伤，贪心先锁增伤，河豚 + 5 号穿透这套常见配进不去。

**现行搜索**：

- 普通路：当前收益从高到低，贪心 + 1-swap + 2-swap。
- 穿透专路：已启用则锁 `main:slot5:penRate` / `set:penRate:8`，在带穿透率的面板上重测副词条再搜，和普通路比最终总伤。两条都关则不跑。
- `gainAsc`、`declared` 已删。每条路只跑一个起点。
- 候选先按最低收益比例筛（默认 0%），过密才用宽度 K 截顶。
- 提升率两条路都用「一个词条都没分」的总伤当分母。

**游戏专用**：仍是外层 8 个口袋（4/5/6 买不买攻生防），只这个按钮才有。付费 5 号和 24% 是口袋之间比总伤；已经买了 5 号攻/生/防的口袋不能再锁 24%。默认勾选全开。

**计算量**：auto 默认 **200000**（原 36000）。36k 时叶释渊 96 命中、46 档会搜到一半停在 39 档。20 万后脚本约 14s（93217/200000）。可手动停止。auto 的候选宽度仍按剩余计算量推导，所以 20 万既是刹车也在改每轮试几条，这两件事还没拆。

**验证**：`test-affix-optimizer.mjs` 126 passed；`test-game-affix-rules.mjs` 13 passed。fixture `zzz-hp/fixtures/pen-rate-alloc/ye-shiyuan-pen-rate.json`（「不带东西」）选出 24+8。锐爆诊断未发现漏解，未加锐爆专路。

相关提交：`042a52d` `d22ff39` `b9e949b` `e23f9b8` `c0c86b7`。未推 GitHub。
