...
| **最优分配改自适应 Beam** | 单路贪心换成多路线并行；三参数「初始候选门槛 / 路线保留比例 / 最大保留路线」+ 快速/均衡/精细预设；游戏 8 口袋全跑 Beam | 见 `feature/affix-alloc-rework`（未提交） |
...
**游戏专用**：预设与高级参数透传给全部 8 个口袋（4/5/6 买不买攻生防），**8 袋都完整跑 Beam**，每袋独立公平预算，最后按总伤取最高。结果给出胜出口袋与 8 袋合计计算量。已经买了 5 号攻/生/防的口袋锁不到 24%（那个口袋没有 `main:slot5:penRate`），只会锁 2 件套 8%。
...
**验证**：`test-affix-optimizer.mjs` 44 passed（含门槛 / 比例 / B 三组扫描与「快速 ≥ 精细 95%」「均衡 ≥ 精细 99%」两道质量闸）；`test-game-affix-rules.mjs` 53 passed（8 口袋实跑、按总伤取最高、8 袋合计、5 号付费袋锁不到 24%）；`scripts/test-*.mjs` 40 个脚本全过；`vue-tsc` / ESLint 过。fixture `zzz-hp/fixtures/pen-rate-alloc/ye-shiyuan-pen-rate.json`（「不带东西」）选出 24+8。
