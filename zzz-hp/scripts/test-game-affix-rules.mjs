/**
 * 游戏专用分配规则：写死口袋 / 付费占用 / 组额度，并实跑 8 口袋 Beam 求解。
...
console.log('\n[游戏专用方案] 5 号付费袋不锁 24% 穿透')
{
  ...
  const paid5 = buildGameAffixBranch({
    entries,
    enabledIds,
    combo: { slot4: 'free', slot5: 'paid', slot6: 'free' },
    extraCost: EXTRA_COST,
  })
  ...
  const free5 = buildGameAffixBranch({
    entries,
    enabledIds,
    combo: { slot4: 'free', slot5: 'free', slot6: 'free' },
    extraCost: EXTRA_COST,
  })
