...
export function computeDamageResult(input: DamageCalcInput): DamageCalcResult {
  ...
  const staggerMultiplier =
    input.enemyInput.staggerMultiplier + input.combatStaggerVulnerable / 100
  ...
  const directDamageExpected =
    generalMultiplier * critMultiplier * Math.max(0, specialMultiplier) * directDmgMultZone
  ...
  // 异常基础期望：通用 × 精通 × 等级（不含异常增伤/倍率/暴击）
  const anomalyBaseExpected = generalMultiplier * masteryZone * levelZone
