...
<template>
  <template v-if="hasEvents">
    ...
    >
      <!-- 选中事件的详细计算过程：渲染在被点事件的正下方（由 OwnerShareBlock 的 event-detail 插槽承接） -->
    ...
    <p v-if="!detail && (summary || skippedEvents?.length)" class="hint">
      在上方「产生者伤害占比」中点选事件，查看该事件的详细计算过程。
    </p>
    <p v-else-if="!summary?.shares.length && !skippedEvents?.length" class="hint">
      当前配置下暂无可用伤害事件，请检查产生角色与局外面板。
    </p>
  </template>
